...
class Client:
    ...
    def getGoodSubmissions(self):
        for s in self.reddit.get_subreddit('test').get_hot(limit=500):
    ...
    def makeCloud(self, text, font=None):
        ...
        words, counts = wordcloud.process_text(text, max_features=200000)
...
class reddit_client:
    ...
    def process_posting_title(self, submission_id, reddit_submission):
        submission_title = vars(reddit_submission)['title']
        rules_broken = []
        
        #submission_title = "BREAKING: FDSF"
        posting_title_rules = self.config['posting_title_rules']
        for rule_id in posting_title_rules:
            #validate each rule using regex
            #print "checking rule", rule_id
            regex_pattern, english_description = posting_title_rules[rule_id]
            
            if (re.match(regex_pattern ,submission_title)):
                rules_broken.append(rule_id)
            
        if len (rules_broken) > 0:
            #reply to posting with rules broken
            reddit_reply = ""
            reddit_reply += self.config["comment_header"] + "\n\n"
            reddit_reply += self.config["posting_title_header"] + "\n\n"
            for rule_broken in rules_broken:
                regex, english_description = posting_title_rules[rule_broken]
                reddit_reply += "- " + english_description + "\n"
            reddit_reply += "\n"
            reddit_reply += self.config["comment_footer"]
            #print reddit_reply
            print "replying to thread: " + submission_id + " \t\tbroken rules: " + str(rules_broken)
            reddit_submission.add_comment(reddit_reply)
        else:
            #no rules are broken
            pass
        
    def process_posting_comments(self, submission_id, reddit_submission):
        pass
        
    def process_posting(self, submission_id):
        print "processing submission", submission_id
        reddit_submission = self.reddit.get_submission(submission_id=submission_id)
        
        self.process_posting_title(submission_id, reddit_submission)
        self.process_posting_comments(submission_id, reddit_submission)
    
def submission(args, config):
    submission_id = args.subId
    
    client = reddit_client(config)
    client.login()
    
    try:
        client.process_posting(submission_id)
...
def main():
    ...
    parser_submission = subparsers.add_parser('submission', help='process comments for a specific submission ID')
    parser_submission.add_argument('subId', type=str, help='the submission id, like "1s2mkq"')
    parser_submission.set_defaults(func=submission)    
    
    config = json.loads(open('reddiquettenator.json').read())
    args = parser.parse_args()
    args.func(args, config)
